...
def constroi_vertices_cano_curvado(raio, pontos_ctrl, n_amostras=50, n_lados=20):
    ...
    normals = []
    anterior = np.array([0,0,1]) if not np.allclose(tangentes[0], [0,0,1]) else np.array([1,0,0])
...
def constroi_faces_sem_tampas(n_amostras, n_lados):
    faces = []
    for i in range(1, n_amostras-1):  # Só entre 1 e n-2 (cano aberto nas pontas)
...
def plotar_cano_curvado(raio, pontos_ctrl, n_amostras=50, n_lados=20, cor="lightgreen"):
    ...
    collection = Poly3DCollection(poly3d, facecolors=cor, edgecolors="k", linewidths=0.4, alpha=0.23)
    ax.add_collection3d(collection)

    ax.view_init(elev=0, azim=0)  # Olha diretamente na vertical
    ...
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_title("Cano Curvado (aberto nas extremidades)")
    plt.tight_layout()
    plt.show()

def campo_cano_curvado(X, Y, Z, caminho, raio, margin=10):
    shape = X.shape
    ...
    field = np.full_like(min_dist, 2 * raio)
    field[mask] = min_dist[mask] - raio
    return field.reshape(shape)

def plotar_cano_curvado_marching_cubes(
    raio, pontos_ctrl, grid_size=120, n_amostras=200, cor="lime"
):
    caminho = curva_bezier(pontos_ctrl, n_amostras)
    min_xyz = caminho.min(axis=0) - raio * 1.2
    max_xyz = caminho.max(axis=0) + raio * 1.2
    ...
    campo = campo_cano_curvado(X, Y, Z, caminho, raio, margin=10)
    ...
    ax = fig.add_subplot(111, projection='3d')
    mesh = Poly3DCollection(verts[faces], facecolor=cor, edgecolor='k', linewidths=0.05, alpha=0.20)
    ...
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_title("Cano Curvado (Marching Cubes, aberto nas extremidades)")
    plt.tight_layout()
    plt.show()
